...
<head>
    ...
    <link rel='stylesheet' type='text/css' media='screen' href="Officers/style_1.css">
...
<body>
    ...
    <div class="container" style="height:900px; margin-left:11%; margin-top:-10px">
        ...
        <div class="finaldiv">
            ...
            <form id="crimeInfo" method="post"
                style="position:absolute;top:140px;background-image: url('police_logo_1.png'); background-repeat:no-repeat;margin-top: 50px; background-size: 90%; width: 50%;height:469px"
                ...
                <button type="submit" class="submitBtn" onclick="submitBtn()" value="upload"
...
if (isset($_POST['submit'])) {
    ...
    // Check if the assigned officer is in the officer table
    $officerQuery = "SELECT * FROM officer WHERE offName = ?";
    $officerStmt = mysqli_prepare($conn, $officerQuery);
    mysqli_stmt_bind_param($officerStmt, "s", $offName);
    mysqli_stmt_execute($officerStmt);
    $officerResult = mysqli_stmt_get_result($officerStmt);
    $officerRows = mysqli_num_rows($officerResult);
    ...
    if ($officerRows > 0) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Criminal Information Inserted Successfully');</script>";
        } else {
            echo "Error: " . mysqli_error($conn);
        }
        mysqli_stmt_close($stmt);
    } else {
        echo "<script>alert('Assigned officer is not in the list');</script>";
    }
